...
public class DownloadManagerImpl
    ...
    /**
     * Pretend this download is complete while not running, even if it has no
     * data. When the torrent starts up, the real complete level will be checked
     * (probably by DiskManager), and if the torrent actually does have missing
     * data at that point, the download will be thrown into error state.
     * <p>
    ...
    protected int getUploadPriority() {
    ...
    public void setPieceCheckingEnabled(
            boolean enabled) {
    ...
    public void setTrackerScrapeResponse(
    ...
    /**
     * Returns the "first" online scrape response found, and its active URL,
     * otherwise one of the failing scrapes
     *
     * @return
     */
    ...
    protected void informWillBeStarted(
            PEPeerManager pm) {
     ...
     * anything in this session, but we determined that the download is complete
     * (usually via startDownload())
     *
    ...
    public void setDataAlreadyAllocated(
    ...
    public void setAZMessagingEnabled(
            boolean enable) {
        az_messaging_enabled = enable;
    }

    public void setCryptoLevel(
            int level) {
        crypto_level = level;
    }
    ...
    public void moveDataFiles(
         ...
         * exists. Perhaps you might think it is slightly unnecessary to check
         * this, but I would prefer this test left in - we want to prevent
         * pausing unnecessarily pausing a running torrent (it fires off
         * listeners, which might try to move the download).
         *
    ...
    private void moveDataFilesSupport(
            File new_parent_dir,
            String new_filename)
            throws DownloadManagerException {
        ...
        if (dm == null || dm.getFiles() == null) {
            ...
            if (old_file.equals(new_save_location)) {
                ...
                for (int i = 0; i < info_files.length; i++) {
                     ...
                     * files_to_move list. Doesn't matter if we include files
                     * which are outside of the file path, the renameFile call
                     * won't try to move those directories anyway.
                     */
                    while (added_entry) {
                        f = f.getParentFile();
    ...
    public LimitedRateGroup[] getRateLimiters(
            boolean upload) {
    ...
    public void fireGlobalManagerEvent(
            int eventType) {
