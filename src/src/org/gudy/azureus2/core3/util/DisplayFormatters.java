...
public class DisplayFormatters {
	...
	static {
		COConfigurationManager.addAndFireParameterListeners(new String[] {
				"config.style.useSIUnits", "config.style.forceSIValues",
				"config.style.useUnitsRateBits", "config.style.doNotUseGB", },
				new ParameterListener() {
					public void parameterChanged(String x) {
						use_si_units = COConfigurationManager
								.getBooleanParameter("config.style.useSIUnits");
						force_si_values = COConfigurationManager
								.getBooleanParameter("config.style.forceSIValues");
						use_units_rate_bits = COConfigurationManager
								.getBooleanParameter("config.style.useUnitsRateBits");
						not_use_GB_TB = COConfigurationManager
								.getBooleanParameter("config.style.doNotUseGB");
		...
		COConfigurationManager.addAndFireParameterListeners(new String[] {
				...
				new ParameterListener() {
					public void parameterChanged(String x) {
						separate_prot_data_stats = COConfigurationManager
								.getBooleanParameter("config.style.separateProtDataStats");
						data_stats_only = COConfigurationManager
								.getBooleanParameter("config.style.dataStatsOnly");
					}
				});
	...
	public static void loadMessages() {
		PeerManager_status_finished = getResourceString(
				"PeerManager.status.finished", "Finished");
		PeerManager_status_finishedin = getResourceString(
				"PeerManager.status.finishedin", "Finished in");
		Formats_units_alot = getResourceString("Formats.units.alot", "A lot");
		discarded = getResourceString("discarded", "discarded");
		ManagerItem_waiting = getResourceString("ManagerItem.waiting",
				"waiting");
		ManagerItem_initializing = getResourceString(
				"ManagerItem.initializing", "initializing");
		ManagerItem_allocating = getResourceString("ManagerItem.allocating",
				"allocating");
		ManagerItem_checking = getResourceString("ManagerItem.checking",
				"checking");
		ManagerItem_finishing = getResourceString("ManagerItem.finishing",
				"finishing");
		ManagerItem_ready = getResourceString("ManagerItem.ready", "ready");
		ManagerItem_downloading = getResourceString("ManagerItem.downloading",
				"downloading");
		ManagerItem_seeding = getResourceString("ManagerItem.seeding",
				"seeding");
		ManagerItem_superseeding = getResourceString(
				"ManagerItem.superseeding", "superseeding");
		ManagerItem_stopping = getResourceString("ManagerItem.stopping",
				"stopping");
		ManagerItem_stopped = getResourceString("ManagerItem.stopped",
				"stopped");
	...
	private static String getResourceString(String key, String def) {
		if (message_text_state == 0) {
			...
			// this fooling around is to permit the use of this class in the
			// absence of the (large) overhead
			// of resource bundles
	...
	public static String formatByteCountToKiBEtc(long n, boolean rate,
		...
		// (7.3*1024 = 7475.2; 7475/1024.0 = 7.2998; trunc(7.2998, 1 prec.) ==
		// 7.2
		//
	...
	public static String formatDataProtByteCountToKiBEtcPerSec(long data,
		...
		if (separate_prot_data_stats) {
			if (data == 0 && prot == 0) {
				...
				return (formatByteCountToKiBEtcPerSec(data) + " ("
						+ formatByteCountToKiBEtcPerSec(prot) + ")");
			}
		} else if (data_stats_only) {
			return (formatByteCountToKiBEtcPerSec(data));
		} else {
	...
	public static String formatByteCountToBitsPerSec(long n) {
		...
		return (formatDecimal(dbl, precision, true, true)
				+ units_bits[unitIndex] + per_sec);
	}
	...
	private static final SimpleDateFormat abs_df = new SimpleDateFormat(
			"yyyy/MM/dd HH:mm:ss");
	...
	public static String formatETA(long eta, boolean abs) {
		...
		if (eta > 0) {
			if (abs
					&& !(eta == Constants.CRAPPY_INFINITY_AS_INT || eta >= Constants.CRAPPY_INFINITE_AS_LONG)) {
		...
		return PeerManager_status_finishedin + " "
	...
	public static String formatDownloaded(DownloadManagerStats stats) {
		...
		if (total_discarded == 0) {
			...
			return formatByteCountToKiBEtc(total_received)
					+ " ( "
					+ DisplayFormatters
							.formatByteCountToKiBEtc(total_discarded) + " "
					+ discarded + " )";
		}
	}
		...
		case DownloadManager.STATE_SEEDING: {
			...
			if (diskManager != null) {
				...
				if (mp != -1) {
					...
					tmp = ManagerItem_moving + ": "
							+ formatPercentFromThousands(mp);
	...
	public static String formatDownloadStatusDefaultLocale(
		...
		case DownloadManager.STATE_INITIALIZING:
			tmp = MessageText
					.getDefaultLocaleString("ManagerItem.initializing");
			break;
		case DownloadManager.STATE_INITIALIZED:
			tmp = MessageText
					.getDefaultLocaleString("ManagerItem.initializing");
		...
		case DownloadManager.STATE_SEEDING:
			if (dm != null && dm.getCompleteRecheckStatus() != -1) {
				...
				tmp = MessageText.getDefaultLocaleString("ManagerItem.seeding")
						+ " + "
						+ MessageText
								.getDefaultLocaleString("ManagerItem.checking")
						+ ": " + formatPercentFromThousands(done);
			} else if (manager.getPeerManager() != null
					&& manager.getPeerManager().isSuperSeedMode()) {

				tmp = MessageText
						.getDefaultLocaleString("ManagerItem.superseeding");
			} else {
				tmp = MessageText.getDefaultLocaleString("ManagerItem.seeding");
			}
			break;
		...
		case DownloadManager.STATE_STOPPED:
			tmp = MessageText
					.getDefaultLocaleString(manager.isPaused() ? "ManagerItem.paused"
							: "ManagerItem.stopped");
			break;
		case DownloadManager.STATE_QUEUED:
		...
		case DownloadManager.STATE_ERROR:
			tmp = MessageText
					.getDefaultLocaleString("ManagerItem.error").concat(": ").concat(manager.getErrorDetails()); //$NON-NLS-1$ //$NON-NLS-2$
			break;
		default:
			tmp = String.valueOf(state);
		}
	...
	public static String formatTimeStamp(long time) {
		...
		sb.append('.');
		sb.append(formatIntToTwoDigits(calendar.get(Calendar.MONTH) + 1)); // 0
																			// based
		sb.append('.');
	...
	/**
	 * Format a real number to the precision specified. Does not round the
	 * number or truncate trailing zeros.
	 * 
	 * @param value
	...
	/**
	 * Attempts vaguely smart string truncation by searching for largest token
	 * and truncating that
	 * 
	 * @param str
	 * @param width
	...
	public static String truncateString(String str, int width) {
		...
		if (max_len >= excess) {
			...
			return (str.substring(0, trim_point - excess) + "..." + str
					.substring(trim_point));
		} else {
	...
	public static void main(String[] args) {
		...
		System.out.println("0.001:"
				+ DisplayFormatters.formatDecimal(0.001, 6, TRUNCZEROS_YES,
						ROUND_NO));
		// should display 0
		System.out.println("0:" + DisplayFormatters.formatDecimal(0, 0));
		// should display 123456
		System.out.println("123456:"
				+ DisplayFormatters.formatDecimal(123456, 0));
		// should display 123456
		System.out.println("123456:"
				+ DisplayFormatters.formatDecimal(123456.999, 0));
