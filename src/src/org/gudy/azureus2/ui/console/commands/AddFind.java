...
/**
 * this class allows the user to add and find torrents. when adding, you may
 * specify an output directory when finding, it will cache the files it finds
 * into the ConsoleInput object so that they can then be added by id
 * 
 * @author tobi, fatal
 */
public class AddFind extends OptionsConsoleCommand {
	...
	public void execute(String commandName, ConsoleInput ci,
		...
		if (!f.isAbsolute()) {
			...
			try {
				...
				throw new AzureusCoreException(
						"exception occurred while converting directory: ./"
								+ outputDir + " to its canonical path");
			}
		}
	...
	/**
	 * attempt to download the torrent specified by 'arg' and save the files in
	 * the torrent to the specified output directory
	 * 
	 * @param ci
	 * @param arg
	...
	protected void addRemote(ConsoleInput ci, String arg, String outputDir) {
		...
		try {
			...
			ci.out.println("An error occurred while downloading torrent: "
					+ e.getMessage());
	...
	/**
	 * attempt a local add (arg may be a directory, a file or a pattern eg:
	 * d:/*.torrent)
	 * 
	 * @param ci
	 * @param arg
	 *            argument - could be directory, file or pattern eg:
	 *            d:\*.torrent
	 * @param outputDir
	 *            directory to save files from torrent to
	 * @param scansubdir
	 ...
	 * @param finding
	 *            if true, don't start downloading the files; simply add them to
	 *            the 'found' list
	 */
	protected void addLocal(ConsoleInput ci, String arg, String outputDir,
			boolean scansubdir, boolean finding) {
		...
		if (test.exists()) {
			if (test.isDirectory()) {
				File[] toadd = FileFinder.findFiles(arg, "*.torrent;*.tor",
						scansubdir);
				if ((toadd != null) && (toadd.length > 0)) {
					addFiles(ci, toadd, finding, outputDir);
				} else {
					ci.adds = null;
					ci.out.println("> Directory '" + arg
							+ "' seems to contain no torrent files.");
				}
			} else {
		...
		// check to see if they are numeric and if so, try and add them from the
		// 'adds' in ci
		try {
			...
			if (ci.adds != null && ci.adds.length > id) {
				...
				ci.out.println("> No such file id '" + id
						+ "'. Try \"add -l\" to list available files");
			}
			return;
		} catch (NumberFormatException e) {
		}
		// last resort - try to process it as a directory/pattern eg:
		// c:/torrents/*.torrent
		...
		if ((files != null) && (files.length > 0)) {
			...
			ci.adds = null;
			ci.out.println("> No files found. Searched for '" + filePattern
					+ "' in '" + dirName + "'");
		}
	}
	...
	protected String transformLocalArgument(String arg) {
		if (arg.startsWith("~/") || arg.equals("~")) {
			arg = StringUtil.current().replaceAll(arg, "~",
	...
	/**
	 * if finding is set, just print the available files and add them to the
	 * 'add' list inside the consoleinput object, otherwise actually add the
	 ...
	 * @param outputDir
	 */
	protected void addFiles(ConsoleInput ci, File[] toadd, boolean finding,
			String outputDir) {
		ci.out.println("> -----");
	...
	/**
	 * prints out the files in the 'add' list that is stored in the console
	 * input object.
	 * 
	 * @param ci
	 */
