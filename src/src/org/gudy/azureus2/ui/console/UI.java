 ...
 */
public class UI extends org.gudy.azureus2.ui.common.UITemplateHeadless
        implements IUserInterface, UIInstanceFactory, UIInstanceBase,
        UIManagerEventListener {

    public static ConsoleInput console = null;
...
//		System.setProperty("java.awt.headless", "true");
    ...
    public void startUI() {
        ...
        if ((!isStarted()) || (console == null) || (!console.isAlive())) {
            ...
            // Unless a system property tells us not to, we'll take stdout and
            // stderr offline.
            if (!"on".equals(System.getProperty("azureus.console.noisy"))) {
                // We'll hide any output to stdout or stderr - we don't want to
                // litter our
                // view.
                ...
                org.gudy.azureus2.core3.logging.Logger
                        .allowLoggingToStdErr(false);
            }

            console = new ConsoleInput("Main", UIConst.getAzureusCore(),
                    System.in, this_out, Boolean.TRUE);
        ...
        PluginInterface pi = UIConst.getAzureusCore().getPluginManager()
        ...
        TorrentDownloaderFactory.initManager(UIConst.getGlobalManager(), true,
                true,
                COConfigurationManager.getStringParameter("Default save path"));

//		if (created_console
//				&& System.getProperty("azureus.console.multiuser") != null) {
...
//		}
    ...
    public void openTorrent(String fileName) {
        ...
        try {
            if (!TorrentUtils.isTorrentFile(fileName)) {//$NON-NLS-1$
                Logger.getLogger("azureus2.ui.console")
                        .error(fileName
                        + " doesn't seem to be a torrent file. Not added.");
                return;
            }
        } catch (Exception e) {
            Logger.getLogger("azureus2.ui.console").error(
        ...
        if (UIConst.getGlobalManager() != null) {
            try {
                String downloadDir = COConfigurationManager
                        .getDirectoryParameter("Default save path");
                console.out.println("Adding torrent: " + fileName
                        + " and saving to " + downloadDir);
                UIConst.getGlobalManager().addDownloadManager(fileName,
                        downloadDir);
            } catch (Exception e) {
                Logger.getLogger("azureus2.ui.console").error(
    ...
    public boolean eventOccurred(UIManagerEvent event) {
        ...
        switch (event.getType()) {
            ...
            case UIManagerEvent.ET_SHOW_TEXT_MESSAGE: // data is String[] - title,
            // message, text
            {
            ...
            case UIManagerEvent.ET_OPEN_TORRENT_VIA_URL: // data is
            // Object[]{URL,URL,Boolean}
            // - { torrent_url,
            // referrer url,
            // auto_download}
            {
            ...
            case UIManagerEvent.ET_PLUGIN_VIEW_MODEL_CREATED: // data is
            // PluginViewModel
            // (or subtype)
            {
                break;
            }
            case UIManagerEvent.ET_PLUGIN_CONFIG_MODEL_CREATED: // data is
            // PluginConfigModel
            // (or subtype)
            {
                break;
            ...
            case UIManagerEvent.ET_PLUGIN_VIEW_MODEL_DESTROYED: // data is
            // PluginViewModel
            // (or subtype)
            {
                break;
            }
            case UIManagerEvent.ET_PLUGIN_CONFIG_MODEL_DESTROYED: // data is
            // PluginConfigModel
            // (or subtype)
            {
                break;
            ...
            case UIManagerEvent.ET_CREATE_TABLE_COLUMN: // data is String[] -
            // table_id, cell_id: result
            // is TableColumn
            {
            ...
            case UIManagerEvent.ET_ADD_TABLE_COLUMN: // data is TableColumn
            // previously created
            {
                break;
            }
            case UIManagerEvent.ET_ADD_TABLE_CONTEXT_MENU_ITEM: // data is
            // TableContextMenuItem
            {
                break;
            }
            case UIManagerEvent.ET_SHOW_CONFIG_SECTION: // data is String - section
            // id
            {
