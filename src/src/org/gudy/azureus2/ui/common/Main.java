...
import net.downloader.utils.HttpUtils;
...
public class Main {
    ...
    private static CommandLine parseCommands(String[] args, boolean constart) {
        ...
        try {
            ...
            Logger.getLogger("azureus2").error(
                    "Parsing failed.  Reason: " + exp.getMessage(), exp);
            if (constart) {
                System.exit(2);
            }
        }
    ...
    public static void initRootLogger() {
        if (Logger.getRootLogger().getAppender("ConsoleAppender") == null) {
            Appender app;
            app = new ConsoleAppender(new PatternLayout(
    ...
    public static void main(String[] args) {
        ...
        // This *has* to be done first as it sets system properties that are
        // read and cached by Java
        ...
        try {
            ...
            if ((start == null)
                    || (start.getServerState() == StartServer.STATE_FAULTY)) {
    ...
    public static void shutdown() {
        ...
        Logger.getLogger("azureus2").fatal(
                "Azureus stopped at " + temp.format(new Date()));
        // System.exit(0); - we don't want to force quit, wait until other
        // threads have completed
        // so that resume data etc is saved....
    }

    public static boolean directLaunch(String[] args, CommandLine commands) {
        // frig to support launch of SWT ui via this means pending a proper
        // rewrite
        // of this stuff
...
//
//            Method main_method = main_class.getMethod("main",
...
//        }
    ...
    public static void processArgs(String[] args, AzureusCore new_core,
        ...
        if (((commands != null) && (args.length > 0)) || (new_core != null)) {
            ...
            if (commands.hasOption('u')) {
                ...
                if (uinames.indexOf(',') == -1) {
                    if (!UIConst.UIS.containsKey(uinames)) {
                        UIConst.UIS.put(uinames,
                                UserInterfaceFactory.getUI(uinames));
                    }
                } else {
                    ...
                    while (stok.hasMoreTokens()) {
                        ...
                        if (!UIConst.UIS.containsKey(uin)) {
                            UIConst.UIS.put(uin,
                                    UserInterfaceFactory.getUI(uin));
                        }
                    }
                }
            } else {
                if (UIConst.UIS.isEmpty() && !commands.hasOption('c')
                    ...
                    UIConst.UIS.put(DEFAULT_UI,
                            UserInterfaceFactory.getUI(DEFAULT_UI));
                }
            }
            ...
            if (new_core != null) {
                ...
                Logger.getLogger("azureus2").fatal(
                        "Azureus started at " + temp.format(UIConst.startTime));
            ...
            try {
                clConsoleInput = Class
                        .forName("org.gudy.azureus2.ui.console.ConsoleInput");

                // change this and you'll need to change the parameters
                // below....

                Class params[] = {String.class, AzureusCore.class,
                    Reader.class, PrintStream.class, Boolean.class};
            ...
            if (commands.hasOption('e')) {
                if (conConsoleInput != null) {
                    try {
                        Object params[] = {commands.getOptionValue('e'),
                            new_core,
                        ...
                        Logger.getLogger("azureus2").error(
                                "Error invocating the script processor: "
                                + e.toString());
                    }
                } else {
                    Logger.getLogger("azureus2")
                            .error("ConsoleInput class not found. You need the console ui package to use '-e'");
                }
            }
            ...
            if (commands.hasOption('c')) {
                if (conConsoleInput != null) {
                    ...
                    comm += "\nlogout\n";
                    Object params[] = {commands.getOptionValue('c'),
                        UIConst.getAzureusCore(), new StringReader(comm),
                        System.out, Boolean.FALSE};
                    try {
                        conConsoleInput.newInstance(params);
                    } catch (Exception e) {
    ...
    public static void openTorrents(String[] torrents) {
        if ((UIConst.UIS != null) && (!UIConst.UIS.isEmpty())
                && (torrents.length > 0)) {
            for (int l = 0; l < torrents.length; l++) {
                ((IUserInterface) UIConst.UIS.values().toArray()[0])
                        .openTorrent(torrents[l]);
            }
        }
    }
    ...
    public static class StartSocket {
        ...
        public StartSocket(String args[]) {
            ...
            try {
                System.out
                        .println("StartSocket: passing startup args to already-running process.");

                // NOTE - this formatting is also used by
                // AzureusCoreSingleInstanceClient and other
                ...
                StringBuffer buffer = new StringBuffer(
                        AzureusCoreSingleInstanceClient.ACCESS_STRING
                        + ";args;");
